...
The Team Awareness Kit (TAK) is a geospatial mapping and situational awareness platform originally developed for the U.S. military. It has since been adopted by first responders, search and rescue teams, disaster relief organizations, and outdoor enthusiasts worldwide. TAK applications display team members' positions on a shared map, allow placement of markers and annotations, and support text messaging—all using the Cursor on Target (CoT) protocol.
...
```mermaid
flowchart LR
    A[iTAK] <--> B[Meshtastic iOS App] <--> C[Mesh Network] <--> D[Other Meshtastic Nodes] <--> E[ATAK/iTAK]
```
...
3. Configure your TAK settings in the app (Settings → TAK Server) and start the TAK Server
4. Connect iTAK to the Meshtastic app's local TAK endpoint by downloading Data Package from the app and importing it into iTAK
