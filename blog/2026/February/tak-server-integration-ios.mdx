...
![One does not simply use Meshtastic and ATAK on iOS](/img/blog/itak-user.png)
...
:::warning[Bandwidth Consideration]
The TAK integration generates more traffic than typical Meshtastic usage. Consider using a higher bandwidth preset like **Short Fast** or **Short Turbo** for optimal performance, especially when sharing frequent position updates or markers.
:::
...
2. Update to the latest Meshtastic iOS app from the App Store
3. Set your node's device role to **TAK** or **TAK Tracker** so it appears in the TAK ecosystem
4. Configure your TAK settings in the app (Settings → Scroll to the bottom: TAK Server) and start the TAK Server
5. Connect iTAK to the Meshtastic app's local TAK endpoint by downloading the Data Package from the app and importing it into iTAK
6. Go off-grid with full situational awareness!
